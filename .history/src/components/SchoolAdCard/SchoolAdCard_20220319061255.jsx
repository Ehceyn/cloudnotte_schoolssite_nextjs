...
function SchoolAdCard({
  ...
  return (
    <>
      <SEO title={name.toCapitazedString()} siteTitle="Cloudnotte Schools" />
      <section className=" border mt-8 md:rounded-lg">
        <div className="flex px-3 xs:px-5 py-5">
          <article>
            <Image
              loading="lazy"
              src={logoUrl ? logoUrl : `/assets/images/school-profile-img.png`}
              width={85}
              height={85}
              objectFit="contain"
          ...
          <article className="pl-2 xs:pl-4">
            <p className="font-bold">{name.toCapitazedString()}</p>
            <p className=" text-sm">
              {state}, {country}
            </p>
