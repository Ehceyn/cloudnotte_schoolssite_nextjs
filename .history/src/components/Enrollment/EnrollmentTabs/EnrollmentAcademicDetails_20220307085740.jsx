...
function EnrollmentAcademicDetails({ admissionProgrammes, display, schoolId }) {
  ...
  const [submitApplicationToDB, { data, loading, error }] = useMutation(
    ...
    {
      variables: {
        studentFirstName: d[0]?.firstName,
        studentLastName: d[0]?.lastName,
        studentMiddleName: d[0]?.middleName,
        studentPhoneNumber: d[0]?.phoneNumber,
        studentEmail: d[0]?.email,
        studentGender: d[0]?.sex,
        studentDateOfBirth: "2006-01-02T15:04:05Z",
        studentCountry: d[0]?.country,
        studentState: d[0]?.state,
        studentLga: d[0]?.lga,
        studentAddress: d[0]?.address,
        studentPassportUrl: d[0]?.PassportUrl,
        parentFirstName: d[1]?.firstName,
        parentLastName: d[1]?.lastName,
        parentMiddleName: d[1]?.middleName,
        parentPhoneNumber: d[1]?.phoneNumber,
        parentEmail: d[1]?.email,
        parentRelationship: d[1]?.relationship,
        parentOccupation: d[1]?.occupation,
        parentCountry: d[1]?.country,
        parentState: d[1]?.state,
        parentLga: d[1]?.lga,
        parentAddress: d[1]?.address,
        parentPassportUrl: d[1]?.PassportUrl,
        programmeId: d[0]?.programmeId?.state[0],
        schoolId: schoolId,
        previousSchoolName: d[2]?.prevSchool,
        previousSchoolLeaveReason: d[2]?.reason,
        healthIssues: d[2]?.health,
        docs: d[2]?.uploads,
      },
    }
  );
