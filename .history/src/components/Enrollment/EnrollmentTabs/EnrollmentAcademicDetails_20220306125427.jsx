...
function EnrollmentAcademicDetails({ admissionProgrammes, display }) {
  ...
  useEffect(() => {
    setTheClass(formik.values.selectClass);
  }, [formik.values.selectClass]);

  // SCHOOL FEES BASED ON CLASS
  const schoolFees = (class_) => {
    if (class_) {
      return admissionProgrammes.filter((i) => i.name === class_).admissionFee;
    }
  };
  ...
  const changeSchoolFee = (e) => {
    ...
    console.log(schoolFees('JSS 1'), 'Jam fee');
  };
