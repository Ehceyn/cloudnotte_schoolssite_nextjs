...
function EnrollmentAcademicDetails({
  ...
  useEffect(() => {
    ...
    if (typeof docForUpload !== "undefined") {
      ...
      setLoaderState({ ...loadingState, [index]: true });
...
setLoadingState(oldLoadingState => ({...oldLoadingState, [index]: false}))          setUploadMessage("Document uploaded successfully");
        })
        .catch((err) => {
                    ...
                    return (
                      <div className="mb-6 w-full " key={index}>
                        <label htmlFor={doc.name}>
                          ...
                          />
                          <article
                            className={`shadow-sm flex items-center justify-center h-20 pl-7 border border-[#E7F0FB] text-gray-900 text-sm rounded-[5px] outline-none focus:ring-[#5f9af2] focus:border-[#5f9af2] w-full p-2.5 bg-[#FFF] cursor-pointer relative ${styles.mini_loader_inner2}`}
                          >
