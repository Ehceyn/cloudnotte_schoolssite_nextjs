...
Best schools in ${state}, Best schools in ${country}, Best schools in ${city}, Schools in ${state}, Schools in ${country}, Schools in ${city}, School in ${state}, School in ${country}, School in ${city}, private schools in ${city}, private schools in ${states.map(
          (state) => {
            return state.name;
          }
        )}, private schools in ${country},  Best private schools in ${city}, Best private schools in ${states.map(
          (state) => {
            return state.name;
          }
        )}, Best private schools in ${country}, Smart schools, cloudnotte schools, admission-ongoing, schools admitting, schools nearby, school with best facilities, secured schools, automated schools, tech schools.
`}
      />
      <section>
              ...
              >
                <article className="bg-[#FFF] border border-[#E7F0FB] pl-0 pr-0 py-3 sm:py-5 w-full capitalize h-fit flex justify-between">
                  <div className=" w-full flex items-center justify-between px-5 md:px-14 ">
                    ...
                    <article className="border rounded-sm px-1 sm:px-2 py-1 sm:py-2 ml-2 h-fit">
                      <a
                        href={`https://api.whatsapp.com/send?phone=2349035829083&text=Hi%20${name},%20I%20came%20across%20your%20school%20on%20Schools.cloudnotte.com.%20I'd%20like%20to%20make%20more%20enquiries`}
                        target="_blank"
                        rel="noopener noreferrer"
                      >
